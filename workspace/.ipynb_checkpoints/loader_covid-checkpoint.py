...
class Loader_Covid():
    ...
    def split_data(self, test_split=0.25, target='Confirmed'):
        ...
        for location in self.dict:
            ...
            test = X_test[target]
            self.dict[location] = {'train': train, 'test':test}
        return self.dict
    ...
    def forecasting(self, steps=None): #None means forecast=test
        axes = []
        for location in self.dict:
            automodel = model.auto_arima(self.dict[location]['train'])
            self.models[location] = automodel
            ax = model.plot_auto_arima(self.dict[location]['train'], self.dict[location]['test'], automodel, steps, location)
            axes.append(ax)
        return axes
    ...
    def diagnostics(self):
        axes = []
        for location in self.dict:
            ax = model.diagnostics_auto_arima(self.models[location], location, self.target)
            axes.append(ax)
        return axes
